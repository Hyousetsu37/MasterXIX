...
import type { CharacterContextValues, CharacterListEntity } from './types';
...
export const CharacterListProvider = ({
  ...
  const [characters, setCharacters] = useState<CharacterListEntity[]>([]);
  ...
  const loadCharacter = useCallback(
    async (characterName = '', targetPage = 0) => {
      try {
        setIsLoading(true);
        setSearchedCharacter(characterName);
        const { characterEntities, info } = await getCharacters({
          page: targetPage + 1,
          characterName: characterName,
        });
        setCharacters(characterEntities);
        setTotalPages(Number(info.pages) ?? 0);
        setPage(targetPage);
      } catch (error: unknown) {
